...
const CHANNELS = [{ id: "general", name: "공정-협의" }, { id: "quality", name: "품질-안전" }, { id: "material", name: "자재-반입" }];
        ...
        <div style={{ background: "#fff", borderRadius: 14, padding: "16px 20px" }}>
          ...
          {subcons.map(s => (
            <div key={s.name} style={{ marginBottom: 14 }}>
              ...
              <div style={{ fontSize: 11, color: "#9CA3AF", marginTop: 3 }}>담당 공종 {s.count}개 · 예산 {fmtM(s.budget)}</div>
            </div>
          ))}
        </div>
        <div style={{ background: "#fff", borderRadius: 14, padding: "16px 20px" }}>
                ...
                {g.acts.map(a => (
                  <div key={a.id} style={{ background: "#FAFAFA", border: `1px solid ${a.critical ? "#FECACA" : "#E5E7EB"}`, borderRadius: 10, padding: "10px 14px", borderLeft: `3px solid ${a.critical ? "#EF4444" : statusColor(a.status)}` }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6, flexWrap: "wrap" }}>
                      ...
                      {a.weather && <Badge label="☁ 기상" bg="#EFF6FF" color="#1D4ED8" />}
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
                      <div style={{ flex: 1, background: "#E5E7EB", borderRadius: 4, height: 10, overflow: "hidden" }}>
                        <div style={{ width: `${a.phys}%`, height: "100%", background: a.critical ? "#EF4444" : statusColor(a.status), borderRadius: 4, transition: "width 0.8s ease" }} />
                      </div>
                    ...
                    <div style={{ display: "flex", gap: 4, marginBottom: 6, flexWrap: "wrap" }}>
                      {a.steps.map((s, i) => (
                        <span key={i} style={{ fontSize: 11, borderRadius: 6, padding: "2px 7px", background: s.done ? "#D1FAE5" : "#F3F4F6", color: s.done ? "#065F46" : "#6B7280", border: `1px solid ${s.done ? "#6EE7B7" : "#E5E7EB"}` }}>
                          {s.done ? "✓ " : ""}{s.name} ({s.w}%)
                        </span>
                      ))}
                    </div>
                    <div style={{ display: "flex", gap: 10, flexWrap: "wrap", fontSize: 11, color: "#6B7280" }}>
                      <span>📅 {a.ps} ~ {a.pf}</span>
                      {a.original_pf && a.original_pf !== a.pf && <span style={{ color: "#EF4444" }}>원래 완료: {a.original_pf}</span>}
                    ...
                    {a.predecessors && a.predecessors.length > 0 && (
                      <div style={{ fontSize: 11, color: "#9CA3AF", marginTop: 4 }}>
                        선행: {a.predecessors.map(p => `${activities.find(x => x.id === p.id)?.name || "ID" + p.id}(${p.type})`).join(" → ")}
                      </div>
                    )}
                  </div>
                ))}
...
function ActivityFormModal({ onClose, onSave, activities, existingGroups }) {
  ...
  const autoWBS = () => {
    const g = (form.group === "직접입력" ? form.group_custom : form.group || "X").slice(0, 3).toUpperCase().replace(/\s/g, "");
    return `A-${form.floor.replace(/\s/g, "")}-${g}-${form.loc.replace(/\s/g, "").slice(0, 6).toUpperCase() || "LOC"}`;
  };
  const generateSteps = async () => {
    if (!form.name) return;
    setAiLoading(true);
    try {
      const r = await claudeComplete(`한국 건설 현장 "${form.name}" 공종의 작업 단계 3~5개와 가중치(합계100). JSON 배열만: [{"name":"단계명","w":숫자}]`);
      const m = r.match(/\[[\s\S]*\]/);
      if (m) setForm(p => ({ ...p, steps: JSON.parse(m[0]) }));
    } catch {}
    setAiLoading(false);
  };
  ...
  const validate = () => {
    ...
    if (!g) e.group = "공종 그룹을 선택하세요";
    if (!form.name) e.name = "공정명을 입력하세요";
    if (!form.loc) e.loc = "위치를 입력하세요";
    if (!form.ps || !form.pf) e.date = "착수/완료일을 입력하세요";
    if (form.ps && form.pf && form.ps > form.pf) e.date = "완료일이 착수일보다 빠릅니다";
    if (!form.plan_qty || Number(form.plan_qty) <= 0) e.plan_qty = "계획 물량을 입력하세요";
    if (!form.pv_budget || Number(form.pv_budget) <= 0) e.pv_budget = "예산을 입력하세요";
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    const g = form.group === "직접입력" ? form.group_custom : form.group;
    const dur = diffDays(form.pf, form.ps);
    const payload = {
      group_name: g, wbs: autoWBS(), name: form.name, floor: form.floor, loc: form.loc,
      subcon: form.subcon, resp: form.resp, ps: form.ps, pf: form.pf, as_: null, af: null,
      bl_s: form.ps, bl_f: form.pf, original_ps: form.ps, original_pf: form.pf,
      orig_dur: dur, plan_qty: Number(form.plan_qty), done_qty: 0, unit: form.unit,
      steps: form.steps.map(s => ({ ...s, done: false })),
      predecessors: form.predecessors,
      pv_budget: Number(form.pv_budget) * 10000, ac: 0,
      risk: form.risk, weather: form.weather, critical: form.critical, delay_days: 0,
    };
    try {
      const [saved] = await sb.post("activities", payload);
      onSave(calcAct(saved));
    } catch (err) { alert("저장 실패: " + err.message); }
    setSaving(false);
  };
  const inputStyle = { width: "100%", border: "1.5px solid #D1D5DB", borderRadius: 8, padding: "8px 12px", fontSize: 13, outline: "none", boxSizing: "border-box", background: "#fff" };
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.45)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
      <div style={{ background: "#fff", borderRadius: 16, width: "100%", maxWidth: 580, maxHeight: "90vh", overflowY: "auto" }}>
        ...
        <div style={{ display: "flex", borderBottom: "1px solid #E5E7EB" }}>
          {["① 기본정보", "② 물량·비용", "③ 작업단계", "④ 선행관계"].map((t, i) => (
            <button key={i} onClick={() => setStep(i + 1)} style={{ flex: 1, padding: "10px 0", border: "none", background: "none", fontSize: 11, fontWeight: step === i + 1 ? 700 : 400, color: step === i + 1 ? NAVY : "#6B7280", borderBottom: step === i + 1 ? `2px solid ${YELLOW}` : "2px solid transparent", cursor: "pointer" }}>{t}</button>
          ))}
        </div>
          ...
          {step === 1 && (
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                <div>
                  <label style={labelStyle}>공종 그룹 *</label>
                  <select value={form.group} onChange={e => set("group", e.target.value)} style={inputStyle}>
                    <option value="">선택하세요</option>
                    {allGroups.map(g => <option key={g} value={g}>{g}</option>)}
                    <option value="직접입력">+ 직접 입력</option>
                  </select>
                  {form.group === "직접입력" && <input value={form.group_custom} onChange={e => set("group_custom", e.target.value)} placeholder="새 공종 그룹명" style={{ ...inputStyle, marginTop: 6 }} />}
                  {errors.group && <div style={errStyle}>{errors.group}</div>}
                </div>
                <div>
                  <label style={labelStyle}>공정명 *</label>
                  <input value={form.name} onChange={e => set("name", e.target.value)} placeholder="예: 501호 외벽 도장" style={inputStyle} />
                  {errors.name && <div style={errStyle}>{errors.name}</div>}
                </div>
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                <div><label style={labelStyle}>층</label><select value={form.floor} onChange={e => set("floor", e.target.value)} style={inputStyle}>{FLOORS.map(f => <option key={f}>{f}</option>)}</select></div>
                <div><label style={labelStyle}>위치 *</label><input value={form.loc} onChange={e => set("loc", e.target.value)} placeholder="예: 501호" style={inputStyle} />{errors.loc && <div style={errStyle}>{errors.loc}</div>}</div>
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                <div><label style={labelStyle}>협력사</label><select value={form.subcon} onChange={e => set("subcon", e.target.value)} style={inputStyle}>{SUBCONS.map(s => <option key={s}>{s}</option>)}</select></div>
                <div><label style={labelStyle}>담당자</label><select value={form.resp} onChange={e => set("resp", e.target.value)} style={inputStyle}>{RESPS.map(r => <option key={r}>{r}</option>)}</select></div>
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                <div><label style={labelStyle}>계획 착수일 *</label><input type="date" value={form.ps} onChange={e => set("ps", e.target.value)} style={inputStyle} /></div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              ...
              {form.steps.map((s, i) => (
                <div key={i} style={{ display: "flex", gap: 8, alignItems: "center" }}>
                  ...
                  <input value={s.name} onChange={e => setStepField(i, "name", e.target.value)} placeholder={`단계 ${i + 1}`} style={{ ...inputStyle, flex: 1 }} />
                  <input type="number" value={s.w} onChange={e => setStepField(i, "w", Number(e.target.value))} style={{ ...inputStyle, width: 60, textAlign: "center" }} />
              ...
              {errors.steps && <div style={errStyle}>{errors.steps}</div>}
            </div>
          )}
          {step === 4 && (
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              ...
              {activities.filter(a => a.name !== form.name).map(a => {
                ...
                return (
                  <div key={a.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 14px", border: `1.5px solid ${selected ? YELLOW : "#E5E7EB"}`, borderRadius: 10, cursor: "pointer", background: selected ? "#FFFBEB" : "#fff" }} onClick={() => togglePred(a.id)}>
                    <input type="checkbox" checked={!!selected} onChange={() => {}} style={{ width: 16, height: 16 }} />
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: 13, fontWeight: 600, color: NAVY }}>{a.name}</div>
                      <div style={{ fontSize: 11, color: "#9CA3AF" }}>{a.ps} ~ {a.pf} · {pct(a.phys)}</div>
                    </div>
                    {selected && (
                      <select value={selected.type} onChange={e => { e.stopPropagation(); setForm(p => ({ ...p, predecessors: p.predecessors.map(x => x.id === a.id ? { ...x, type: e.target.value } : x) })); }} style={{ border: "1px solid #D1D5DB", borderRadius: 6, padding: "3px 6px", fontSize: 12 }} onClick={e => e.stopPropagation()}>
...
function ApprovalPanel({ activities, setActivities, progressReports, setProgressReports, issues, setIssues, setToast }) {
  ...
  const handleReject = async (report) => {
    try {
      await sb.patch("progress_reports", report.id, { status: "rejected" });
      setProgressReports(p => p.map(r => r.id === report.id ? { ...r, status: "rejected" } : r));
      setToast("반려되었습니다");
      ...
      {pending.map(report => {
        ...
        return (
          <div key={report.id} style={{ background: flash ? "#D1FAE5" : "#fff", border: `1.5px solid ${flash ? "#10B981" : YELLOW}`, borderRadius: 14, padding: "16px 20px", marginBottom: 14, transition: "background 0.3s" }}>
            ...
            <div style={{ fontWeight: 700, fontSize: 15, color: NAVY }}>{act?.name}</div>
              ...
              {CHAIN_ROLES.map((rank, i) => {
                ...
                return (
                  <div key={i} style={{ display: "flex", alignItems: "center" }}>
                    <div style={{ background: done ? "#10B981" : active ? YELLOW : "#F3F4F6", border: `1px solid ${done ? "#10B981" : active ? YELLOW : "#D1D5DB"}`, borderRadius: 6, padding: "3px 6px", textAlign: "center", minWidth: 44 }}>
                      <div style={{ fontSize: 10, color: done ? "#fff" : active ? NAVY : "#6B7280" }}>{rank}</div>
                      <div style={{ fontSize: 10, fontWeight: 600, color: done ? "#fff" : active ? NAVY : "#9CA3AF" }}>{done ? "✓" : active ? "⏳" : "—"}</div>
                      <div style={{ fontSize: 9, color: done ? "#d1fae5" : active ? "#78350f" : "#9CA3AF" }}>{CHAIN_NAMES[i]}</div>
                    </div>
                    {i < 4 && <div style={{ width: 5, height: 1, background: "#D1D5DB" }} />}
                  </div>
                );
              })}
            </div>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
      ...
      <div style={{ padding: "12px 20px", borderTop: "1px solid #E5E7EB", display: "flex", gap: 8 }}>
        <input value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === "Enter" && handleSend()} placeholder={`# ${channelName}에 메시지 보내기`} style={{ flex: 1, border: "1.5px solid #D1D5DB", borderRadius: 12, padding: "11px 16px", fontSize: 14, outline: "none" }} />
        <button onClick={handleSend} style={{ background: NAVY, border: "none", borderRadius: 12, padding: "0 20px", fontWeight: 700, fontSize: 14, color: "#fff", cursor: "pointer" }}>전송</button>
      </div>
    </div>
  );
}
...
JSON: {"matched_activity_id":<숫자|null>,"new_done_qty":<숫자>,"workers":<숫자>,"special_note":"<특이사항>","delay_days":<지연일수,없으면0>,"delay_reason":"<지연원인,없으면빈문자열>","summary":"<한줄>","ai_message":"<응답>","needs_clarification":false}`;
  ...
  const handleReportSubmit = async () => {
    ...
    setInput("");
    const uid = Date.now();
    setChatMessages(p => [...p, { id: uid, role: "user", content: msg }]);
    setChatMessages(p => [...p, { id: uid + 1, role: "loading", content: "AI 분석 중..." }]);
    setLoading(true);
    try {
      ...
      if (res.needs_clarification || !matched) {
        setChatMessages(p => [...p, { id: uid + 2, role: "ai", content: res.ai_message || "공정을 찾지 못했습니다." }]);
      } else {
      ...
      setChatMessages(p => p.filter(m => m.id !== uid + 1));
      setChatMessages(p => [...p, { id: uid + 2, role: "ai", content: "오류가 발생했습니다." }]);
    }
    setLoading(false);
  };
    ...
    <div style={{ maxWidth: 420, margin: "0 auto", display: "flex", flexDirection: "column", height: "calc(100vh - 56px)", background: "#FAFAFA" }}>
      ...
      <div style={{ display: "flex", background: "#fff", borderBottom: "1px solid #E5E7EB" }}>
        {[{ id: "report", label: "📋 작업 보고" }, { id: "chat", label: "💬 팀 채팅" }].map(t => (
          <button key={t.id} onClick={() => setTab(t.id)} style={{ flex: 1, padding: "11px 0", border: "none", background: "none", fontWeight: tab === t.id ? 700 : 400, fontSize: 13, color: tab === t.id ? NAVY : "#6B7280", borderBottom: tab === t.id ? `2px solid ${YELLOW}` : "2px solid transparent", cursor: "pointer" }}>{t.label}</button>
        ))}
      </div>
      {tab === "report" ? (
        <>
          <div style={{ flex: 1, overflowY: "auto", padding: "14px 12px", display: "flex", flexDirection: "column", gap: 10 }}>
                ...
                {activities.filter(a => a.phys < 100 && a.as_).map(a => {
                  ...
                  return (
                    <div key={a.id} style={{ background: "rgba(255,255,255,0.08)", borderRadius: 10, padding: "10px 14px", minWidth: 130, flexShrink: 0 }}>
                      ...
                      <div style={{ fontSize: 18, fontWeight: 800, color: YELLOW }}>{daily_target}<span style={{ fontSize: 11, fontWeight: 400, color: "#9CA3AF" }}> {a.unit}</span></div>
                      <div style={{ fontSize: 10, color: "#6B7280", marginTop: 2 }}>잔여 {rem_days}일 · {a.done_qty}/{a.plan_qty}{a.unit}</div>
                    </div>
                  );
                })}
            ...
            {chatMessages.map(m => {
              ...
              if (m.role === "user") return <div key={m.id} style={{ display: "flex", justifyContent: "flex-end" }}><div style={{ background: "#374151", color: "#fff", borderRadius: "18px 18px 4px 18px", padding: "10px 14px", maxWidth: "80%", fontSize: 14, lineHeight: 1.5 }}>{m.content}</div></div>;
            ...
            {pendingReport && (
              <div style={{ background: "#fff", border: `2px solid ${YELLOW}`, borderRadius: 14, padding: "14px 16px" }}>
                ...
                {pendingReport.delay_days > 0 && (
                  <div style={{ background: "#FEE2E2", border: "1px solid #FECACA", borderRadius: 8, padding: "8px 12px", marginBottom: 8 }}>
                    <div style={{ fontSize: 12, fontWeight: 700, color: "#991B1B" }}>🚨 공기 지연 감지: +{pendingReport.delay_days}일</div>
                    {pendingReport.delay_reason && <div style={{ fontSize: 11, color: "#EF4444", marginTop: 2 }}>원인: {pendingReport.delay_reason}</div>}
        ...
        <>
          <div style={{ display: "flex", gap: 4, padding: "8px 12px", background: "#fff", borderBottom: "1px solid #E5E7EB" }}>
      ...
      <div style={{ width: 220, background: NAVY, color: "#fff", flexShrink: 0, display: "flex", flexDirection: "column", padding: "20px 0" }}>
        ...
        <div style={{ padding: "14px 12px", flex: 1 }}>
          {SIDEBAR_ITEMS.map(item => {
            ...
            return (
              <div key={item.id} onClick={() => setActiveMenu(item.id)} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "8px 10px", borderRadius: 8, marginBottom: 2, background: isActive ? "rgba(255,184,0,0.18)" : "transparent", cursor: "pointer" }}>
                <span style={{ fontSize: 13, color: isActive ? YELLOW : item.type === "channel" ? "#9CA3AF" : "#D1D5DB" }}>
...
export default function App() {
  ...
  if (dbLoading) return (
    <div style={{ fontFamily: "'Pretendard','Apple SD Gothic Neo',sans-serif", minHeight: "100vh", background: "#FAFAFA", display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", gap: 16 }}>
      ...
      <div style={{ fontSize: 14, color: "#6B7280" }}>Supabase에서 데이터 불러오는 중...</div>
    </div>
  );
    ...
    <div style={{ fontFamily: "'Pretendard','Apple SD Gothic Neo','Noto Sans KR',sans-serif", minHeight: "100vh", background: "#FAFAFA" }}>
      <div style={{ background: NAVY, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 24px", height: 56 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <span style={{ color: "#fff", fontWeight: 700, fontSize: 15 }}>S-PMIS Collab</span>
          <span style={{ fontSize: 10, color: "#10B981", background: "rgba(16,185,129,0.15)", borderRadius: 6, padding: "2px 8px" }}>● Supabase</span>
        </div>
        <div style={{ display: "flex", gap: 6 }}>
          <button onClick={() => setView("mobile")} style={{ background: view === "mobile" ? YELLOW : "rgba(255,255,255,0.1)", color: view === "mobile" ? NAVY : "#fff", border: "none", borderRadius: 8, padding: "7px 16px", fontWeight: 600, fontSize: 13, cursor: "pointer" }}>📱 모바일 (현장 반장)</button>
