  ...
  return (
    <div style={{ padding: 20, overflowY: "auto", height: "100%" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
        ...
        <div style={{ display: "flex", background: T.bg, borderRadius: 10, padding: 4 }}>
      ...
      <div style={{ display: "flex", gap: 12, marginBottom: 20, flexWrap: "wrap" }}>
        {[
          ...
          <div key={k.label} style={{ flex: 1, minWidth: 100, background: T.card, borderRadius: 12, padding: "14px 16px", boxShadow: "0 1px 4px rgba(0,0,0,0.06)" }}>
            <div style={{ fontSize: 11, color: T.sub, marginBottom: 4 }}>{k.label}</div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 16 }}>
        ...
        <div style={{ background: T.card, borderRadius: 14, padding: "16px 20px", boxShadow: "0 1px 4px rgba(0,0,0,0.06)" }}>
          ...
          {companies.length === 0 && <div style={{ color: T.sub, fontSize: 13 }}>데이터 없음</div>}
          {companies.map(c => (
            <div key={c.name} style={{ marginBottom: 14 }}>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                <span style={{ fontSize: 13, fontWeight: 600, color: T.text }}>{c.name}</span>
                <div style={{ display: "flex", gap: 8 }}>
                  <span style={{ fontSize: 11, color: T.sub }}>{c.count}건</span>
                  {c.delayed > 0 && <span style={{ fontSize: 11, color: "#EF4444", fontWeight: 700 }}>지연 {c.delayed}건</span>}
                </div>
              </div>
              <div style={{ background: T.bg, borderRadius: 4, height: 12, overflow: "hidden", position: "relative" }}>
        ...
        <div style={{ background: T.card, borderRadius: 14, padding: "16px 20px", boxShadow: "0 1px 4px rgba(0,0,0,0.06)" }}>
          ...
          <div style={{ fontSize: 11, color: T.sub, marginBottom: 12 }}>피크 시간대: {String(peakHour).padStart(2, "0")}:00 ({hourMap[peakHour]}건)</div>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", marginTop: 4 }}>
            {[0, 6, 12, 18, 23].map(h => (
              <span key={h} style={{ fontSize: 9, color: T.sub }}>{String(h).padStart(2, "0")}시</span>
            ))}
          </div>
        </div>
      </div>
      ...
      <div style={{ background: T.card, borderRadius: 14, padding: "16px 20px", boxShadow: "0 1px 4px rgba(0,0,0,0.06)" }}>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
          ...
          <tbody>
            {filtered.length === 0 && (
              <tr><td colSpan={8} style={{ padding: "20px", textAlign: "center", color: T.sub }}>데이터 없음</td></tr>
            )}
