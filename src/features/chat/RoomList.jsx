    ...
    <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
      ...
      <div style={{ padding: "16px 20px", borderBottom: `1px solid ${T.border}`, fontWeight: 700, fontSize: 16, color: T.text, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        <button onClick={() => { setShowNewChat(true); setShowGroupForm(false); }}
          style={{ background: T.blue, border: "none", borderRadius: 8, padding: "6px 12px", fontSize: 12, fontWeight: 700, color: "#fff", cursor: "pointer" }}>
          + 새 채팅
        </button>
      </div>
      ...
      {showNewChat && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.45)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
          <div style={{ background: T.card, borderRadius: 16, width: "100%", maxWidth: 400, maxHeight: "80vh", overflowY: "auto" }}>
            ...
            {!showGroupForm ? (
              <div style={{ padding: 20 }}>
                ...
                <div onClick={() => setShowGroupForm(true)}
                  style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 16px", borderRadius: 10, background: T.bg, border: `1px solid ${T.border}`, cursor: "pointer", marginBottom: 16 }}
                  onMouseEnter={e => e.currentTarget.style.background = "#F3F4F6"}
                  onMouseLeave={e => e.currentTarget.style.background = "#F9FAFB"}>
                  <div style={{ width: 40, height: 40, borderRadius: "50%", background: T.blue, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18 }}>👥</div>
                  <div>
                    <div style={{ fontWeight: 700, fontSize: 14, color: T.text }}>그룹 채팅</div>
                    <div style={{ fontSize: 12, color: T.sub }}>여러 명과 대화</div>
                  </div>
                </div>

                {/* 1:1 채팅 유저 목록 */}
                <div style={{ fontSize: 12, color: T.sub, fontWeight: 600, marginBottom: 8 }}>1:1 채팅</div>
                {otherProfiles.length === 0 && <div style={{ color: T.sub, fontSize: 13 }}>다른 사용자가 없습니다</div>}
                {otherProfiles.map(p => (
                  <div key={p.id} onClick={() => !creating && handleDirectChat(p)}
                    ...
                    <div style={{ width: 40, height: 40, borderRadius: "50%", background: T.border, display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, fontSize: 16, color: T.text }}>{p.name?.[0]}</div>
                    <div>
                      <div style={{ fontWeight: 600, fontSize: 14, color: T.text }}>{p.name}</div>
                      <div style={{ fontSize: 12, color: T.sub }}>{p.role}</div>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div style={{ padding: 20 }}>
                <div style={{ marginBottom: 16 }}>
                  <label style={{ fontSize: 12, fontWeight: 600, color: T.text, display: "block", marginBottom: 4 }}>그룹 이름 *</label>
                  <input value={groupName} onChange={e => setGroupName(e.target.value)}
                    placeholder="예: 현장 관리팀"
                    style={{ width: "100%", border: `1.5px solid ${T.border}`, borderRadius: 8, padding: "9px 12px", fontSize: 14, outline: "none", boxSizing: "border-box" }} />
                </div>
                <div style={{ fontSize: 12, fontWeight: 600, color: T.text, marginBottom: 8 }}>참여자 선택</div>
                {otherProfiles.map(p => {
                  ...
                  return (
                    <div key={p.id} onClick={() => setSelectedUsers(prev => selected ? prev.filter(u => u.id !== p.id) : [...prev, p])}
                      style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 16px", borderRadius: 10, cursor: "pointer", marginBottom: 6, background: selected ? "#EFF6FF" : "transparent", border: `1.5px solid ${selected ? T.blue : "transparent"}` }}>
                      <div style={{ width: 36, height: 36, borderRadius: "50%", background: selected ? T.blue : "#E5E7EB", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, fontSize: 14, color: selected ? "#fff" : "#374151" }}>{p.name?.[0]}</div>
                      <div style={{ flex: 1 }}>
                        <div style={{ fontWeight: 600, fontSize: 13, color: T.text }}>{p.name}</div>
                        <div style={{ fontSize: 11, color: T.sub }}>{p.role}</div>
                      </div>
                      {selected && <span style={{ color: T.blue, fontSize: 18 }}>✓</span>}
                    </div>
                  );
                })}
                <div style={{ display: "flex", gap: 8, marginTop: 16 }}>
                  <button onClick={() => setShowGroupForm(false)}
                    style={{ flex: 1, background: T.bg, border: "none", borderRadius: 10, padding: "10px 0", fontSize: 13, color: T.text, cursor: "pointer" }}>← 뒤로</button>
                  <button onClick={handleGroupChat} disabled={creating}
                    style={{ flex: 2, background: T.blue, border: "none", borderRadius: 10, padding: "10px 0", fontSize: 13, fontWeight: 700, color: "#fff", cursor: "pointer" }}>
      ...
      <div style={{ flex: 1, overflowY: "auto" }}>
        {rooms.length === 0 && (
          <div style={{ textAlign: "center", padding: "56px 20px" }}>
            <div style={{ width: 56, height: 56, borderRadius: "50%", background: `${T.blue}14`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 26, margin: "0 auto 14px" }}>💬</div>
            <div style={{ fontWeight: 700, fontSize: 15, color: T.text, marginBottom: 4 }}>아직 채팅방이 없어요</div>
            <div style={{ fontSize: 13, color: T.sub }}>새 채팅을 시작해보세요</div>
          </div>
        )}
        {rooms.map(room => {
